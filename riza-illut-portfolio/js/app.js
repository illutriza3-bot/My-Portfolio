(function () {
  ...
  function initFeedbackForm() {
    ...
    const endpoint = (SITE_DATA.person.feedbackFormEndpoint || "").trim();
    ...
    form.addEventListener("submit", async (e) => {
      ...
      // No Formspree endpoint configured yet — fall back to opening an
      // email draft, same as before.
      if (!endpoint) {
        sendViaMailto();
        return;
      }
      ...
      if (!name || !quote) {
        showStatus("Please add at least your name and some feedback.", true);
        return;
      }
      ...
      try {
        const response = await fetch(endpoint, {
          ...
          body: new URLSearchParams({
            ...
            role: role,
            feedback: quote,
            _subject: "Portfolio feedback from " + name
          })
        });

        if (response.ok) {
          showStatus("Thank you — your feedback was sent.", false);
          form.reset();
          updatePreview();
        } else {
          showStatus("Something went wrong sending that. Falling back to email instead…", true);
          sendViaMailto();
        }
      } catch (err) {
        showStatus("Couldn't reach the feedback service. Falling back to email instead…", true);
        sendViaMailto();
      } finally {
